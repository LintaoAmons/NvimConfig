...
local function create_floating_file(location, opts)
  ...
  local contents = vim.api.nvim_buf_get_lines(
    ...
    range.start.line,
    math.min(
      range["end"].line + 1 + (opts.context or default_config.max_height),
      range.start.line + (opts.max_height or default_config.max_height)
    ),
    false
  )
  ...
  opts = vim.lsp.util.make_floating_popup_options(
    if_nil(width, default_config.max_width),
    if_nil(height, default_config.max_height),
    opts
  )
